...
# ---------------------------------------------------------------------------------
def Calculate_DEM_SimpCase(DEMInput, SlopeInput, nrows_org, nrows, ncols, cellsize):
    """
    Calculating DEM of simplified model.
    Parameters
    ----------
    DEMInput : Array
...
def RFR(nrows, ncols, cellsize, Maxrix_Directory, CorrLengthX, CorrLengthY, ParMean, ParCoV, DistType="N", SaveMat="NO"):
    ...
    if CorrLengthX == 0 or CorrLengthY == 0:
        ...
        Name = "Cell_%d_%d_Size_%d" % (nrows, ncols, cellsize) + "_Theta_%d_%d.npy" % (
            CorrLengthX,
            CorrLengthY,
        )
...
def StepwiseRFRv2(nrows, ncols, cellsize, CorrLengthX, CorrLengthY, ParMean, ParCoV, Maxrix_Directory, DistType="N", SaveMat="NO", *args):  ## OK
    ...
    NameX = "Cell_%d_%d_Size_%d" % (
        nrows,
        ncols,
        cellsize,
    ) + "_Theta_%d_%d_X_Dir.npy" % (CorrLengthX, CorrLengthY)
    NameY = "Cell_%d_%d_Size_%d" % (
        nrows,
        ncols,
        cellsize,
    ) + "_Theta_%d_%d_Y_Dir.npy" % (CorrLengthX, CorrLengthY)
...
def CellsInsideEllV2(nrows, ncols, cellsize, DEMInput, EllCenterX, EllCenterY, EllDEMCenter, EllAlpha, EllBeta, Ella, Ellb, Ellc, Ellz):
    ...
    ## Coordinates (e'')
    CoorEll11 = np.concatenate(
        (
            CoorEll[:, [0, 1]],
            np.reshape(x11, (np.shape(CoorG2)[0], 1)),
            np.reshape(y11, (np.shape(CoorG2)[0], 1)),
            np.reshape(z11, (np.shape(CoorG2)[0], 1)),
        ),
        axis=1,
    )
    ...
    for i in range(np.shape(Parameter_CoVs)[0]):
        ...
        for CurrentZoneIndex in ZoneInd_SpaVariable:
            ...
            if RanFieldMethod == "SCMD":
                FieldData = StepwiseRFRv2(
                    nrows,
                    ncols,
                    cellsize,
                    int(Parameter_CorrLenX[i, 0, CurrentZoneIndex]),
                    int(Parameter_CorrLenY[i, 0, CurrentZoneIndex]),
                    Parameter_Means[i, 0, CurrentZoneIndex],
                    Parameter_CoVs[i, 0, CurrentZoneIndex],
                    Maxrix_Directory,
                    Parameter_Dist[i, 0, CurrentZoneIndex],
                    SaveMat,
                    args,
                )
...
def FSCalcEllipsoid_v1_0_SingleRrocess(AnalysisType, FSCalType, RanFieldMethod, InZone, SubDisNum, Results_Directory, Code_Directory, Maxrix_Directory, nrows, ncols, nel, cellsize, EllParam, MCnumber, Parameter_Means, Parameter_CoVs, Parameter_Dist, Parameter_CorrLenX, Parameter_CorrLenY, SaveMat, ZoneInput, SlopeInput, ZmaxArg, ZmaxInput, DEMInput, HwInput, rizeroInput, riInp, AspectInput, TimeToAnalyse, NoData, ProblemName=""):
    ...
    for MC_current in range(MCnumber):  # One MC analysis in each for loop
        ...
        for i in range(np.shape(InZone)[0]):  ## (InZone change)
            ...
            indexes, FS, FS_All_MC = EllipsoidFSWithSubDis_v1_0_SingleProcess(
                FS_All_MC,
                MC_current,
                MCnumber,
                AnalysisType,
...
def FSCalcEllipsoid_v1_0_MutiProcess(
    ...
    if __name__ == "Functions_3DPLS_v1_1":
        ...
        for i in range(TOTAL_PROCESSES_MC):
            ...
            if Multiprocessing_Option == "C-MP-SP":
                ...
                p_mc = Process(
                    target=MCRun_v1_0_SingleProcess,
                    args=(
                        queue_mc,
                        FS_All_MC,
                        AnalysisType,
            ...
            if Multiprocessing_Option == "C-MP-MP":
                print("Run option:", Multiprocessing_Option)
                p_mc = Process(
                    target=MCRun_v1_0_MultiProcess,
                    args=(
                        queue_mc,
                        FS_All_MC,
                        TOTAL_PROCESSES_ELL,
        ...
        for _ in range(TOTAL_PROCESSES_MC):
            queue_mc.put(None)
        for p_mc in processes_mc:
            p_mc.join()
    ...
    while queue_data_mc is not None:
        ...
        if __name__ == "Functions_3DPLS_v1_1":
            ...
            for i in range(TOTAL_PROCESSES_ELL):  ## For Processes
                p_ell = Process(
                    target=EllipsoidFSWithSubDis_v1_0_MultiProcess,
                    args=(
                        queue_ell,
                        FS_All_MC,
                        AnalysisType,
            ...
            for _ in range(TOTAL_PROCESSES_ELL):
                queue_ell.put(None)
            for p_ell in processes_ell:
    ...
    while queue_data_mc is not None:
        ...
        if __name__ == "Functions_3DPLS_v1_1":
            ...
            for i in range(TOTAL_THREADS_ELL):  ## For Processes
                t_ell = Thread(
                    target=EllipsoidFSWithSubDis_v1_0_MultiThread,
                    args=(
                        queue_ell,
                        FS_All_MC,
                        AnalysisType,
            ...
            for _ in range(TOTAL_THREADS_ELL):
                queue_ell.put(None)
            for t_ell in threads_ell:
...
def EllipsoidFSWithSubDis_v1_0_SingleProcess(FS_All_MC, MC_current, MCnumber, AnalysisType, FSCalType, SubDisNum, nrows, ncols, nel, cellsize, Parameter_Fields, EllParam, EllRow, EllColumn, SlopeInput, ZmaxInput, DEMInput, HwInput, rizeroInput, riInp, AspectInput, TimeToAnalyse, NoData, ProblemName=""):
    ...
    if EllAlpha_Calc == "Yes":
        ...
        EllAlpha = np.nanmean(
            AspectInput[
                Cell_Inside_Circle[:, 0].astype(int),
                Cell_Inside_Circle[:, 1].astype(int),
            ]
        )
    ...
    CoorEll1 = np.concatenate(
        (
            CoorEll[:, [0, 1]],
            np.reshape(x1, (np.shape(CoorG)[0], 1)),
            np.reshape(y1, (np.shape(CoorG)[0], 1)),
        ),
        axis=1,
    )
    ...
    indexes, CellsInside = CellsInsideEllV2(
        nrows,
        ncols,
        cellsize,
        DEMInput,
    ...
    zvalues = -np.reshape(
        calz(xvalues[:, 0], yvalues[:, 0], Ella, Ellb, Ellc),
        (np.shape(CellsInside)[0], 1),
    )
    ...
    for i in range(np.shape(CellsInsideE11)[0]):
        GradientsE11.append(
            (
                2 * (CellsInsideE11[i, 2]) / Ella**2,
                2 * (CellsInsideE11[i, 3]) / Ellb**2,
                2 * (CellsInsideE11[i, 4]) / Ellc**2,
            )
        )
                            ...
                            np.sin(
                                np.radians(
                                    (
                                        SlopeInput[
                                            (
                                                CellsInside[:, 0].astype(int),
                                                CellsInside[:, 1].astype(int),
                                            )
                                        ].T[condzmax]
                                    )
                                )
    ...
    if ProblemName == "Pr3S2Dry" or ProblemName == "Pr3S2Wet":
        ...
        tempvalues[condzmax] = np.ones((np.shape(condzmax)[0], 1)) * 10.0
    ...
    if AnalysisType == "Drained":
        PoreWaterForce = HydrologyModel_v1_0(
            TimeToAnalyse,
            CellsInside,
            A,
            HwInput,
    ...
    if AnalysisType == "Drained":
        ...
        if FSCalType == "Normal3D":
            ...
            FS3D = []
            for PoreWaterForceCurrent in PoreWaterForce:
                FS3D_current = FSNormal3D(
                    c,
                    phi,
                    A,
                    Weight,
                    PoreWaterForceCurrent,
                    Theta,
                    ThetaAvr,
                    AngleTangentXZE1,
                )
...
def EllipsoidFSWithSubDis_v1_0_MultiProcess(queue, FS_All_MC, AnalysisType, MCnumber, FSCalType, SubDisNum, nrows, ncols, nel, cellsize, Parameter_Fields, EllParam, SlopeInput, ZmaxInput, DEMInput, HwInput, rizeroInput, riInp, AspectInput, TimeToAnalyse, NoData, ProblemName=""):
    ...
    while queue_data is not None:
        ...
        print(
            "************************************************************ MC:",
            int(queue_data[4]),
            "  Ellipsoid center location:",
            nrows_org * ncols_org * MC_current + EllRow * ncols_org + EllColumn,
            "  FS: ",
            np.round(FS3D, 5),
        )
...
def Ellipsoid_Generate_Main(InZone, SubDisNum, nrows, ncols, nel, cellsize, EllParam, SlopeInput, ZmaxInput, DEMInput, AspectInput, NoData, ProblemName=""):
    ...
    for i in range(np.shape(InZone)[0]):  ## (InZone change)
        ...
        # t_start = time.time() ## Time before FS calculation for 1 ellipdoidal sliding surface
        Inf_ellipsoid = Ellipsoid_Generate(
            count,
            SubDisNum,
            nrows,
            ncols,
...
def Ellipsoid_Generate_Main_Multi(TOTAL_PROCESSES_EllGen, InZone, SubDisNum, nrows, ncols, nel, cellsize, EllParam, SlopeInput, ZmaxInput, DEMInput, AspectInput, NoData, ProblemName=""):
    ...
    if __name__ == "Functions_3DPLS_v1_1":
        ...
        for i in range(TOTAL_PROCESSES_EllGen):
            p_EllGen = Process(
                target=Ellipsoid_Generate_Multi,
                args=(
                    queue_EllGen,
                    shared_list,
                    nrows,
        ...
        for _ in range(TOTAL_PROCESSES_EllGen):
            queue_EllGen.put(None)
        for p_EllGen in processes_EllGen:
            p_EllGen.join()  ## For Processes
...
def IndMC_FS(AllInf_sorted, FS_All_MC, MC_current, AnalysisType, FSCalType, RanFieldMethod, InZone, MCnumber, Results_Directory, Maxrix_Directory, nrows, ncols, cellsize, Parameter_Means, Parameter_CoVs, Parameter_Dist, Parameter_CorrLenX, Parameter_CorrLenY, SaveMat, SlopeInput, ZoneInput, HwInput, rizeroInput, riInp, TimeToAnalyse, NoData, ProblemName=""):
    ...
    for EllCurr in range(np.shape(AllInf_sorted)[0]):
        ...
        if ProblemName == "Pr3S2Dry" or ProblemName == "Pr3S2Wet":
            ...
            # phi[indexes][condzmax] = np.zeros((9520, 1))
            tempvalues = phi[indexes][:]
            tempvalues[condzmax] = np.ones((np.shape(condzmax)[1])) * 10.0
            phi[indexes] = tempvalues
            phi = np.reshape(phi, (nrows, ncols))
        ## !!!
...
def IndMC_Main_Multi(TOTAL_PROCESSES_IndMC, AllInf_sorted, AnalysisType, FSCalType, RanFieldMethod, InZone, Results_Directory, Maxrix_Directory, nrows, ncols, cellsize, MCnumber, Parameter_Means, Parameter_CoVs, Parameter_Dist, Parameter_CorrLenX, Parameter_CorrLenY, SaveMat, SlopeInput, ZoneInput, HwInput, rizeroInput, riInp, TimeToAnalyse, NoData, ProblemName=""):
    ...
    if __name__ == "Functions_3DPLS_v1_1":
        ...
        for _ in range(TOTAL_PROCESSES_IndMC):
            queue_mc_ind.put(None)
        for p_mc_ind in processes_mc_ind:
            p_mc_ind.join()
...
def IndMC_FS_Multi(queue_mc_ind, AllInf_sorted, AnalysisType, FSCalType, RanFieldMethod, InZone, MCnumber, Results_Directory, Maxrix_Directory, nrows, ncols, cellsize, Parameter_Means, Parameter_CoVs, Parameter_Dist, Parameter_CorrLenX, Parameter_CorrLenY, SaveMat, SlopeInput, ZoneInput, HwInput, rizeroInput, riInp, TimeToAnalyse, NoData, ProblemName=""):
    ...
    while queue_data_mc_ind is not None:
        ...
        for EllCurr in range(np.shape(AllInf_sorted)[0]):
            ...
            ## Keep the original values
            SlopeInput, HwInput, rizeroInput = (
                SlopeInput_org,
                HwInput_org,
                rizeroInput_org,
            )
